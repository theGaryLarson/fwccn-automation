...
export function regexReplacer(key, value) {
    if (value instanceof RegExp) {
        return value.toString(); // Convert RegExp to its string representation
    }
    return value;
}
 ...
 * @param targetAddress address; can be streetAddress1 or an addition of streetAddress2 and zip code consecutively
        ...
        return {
            ...(isIdGroup
                    ? { idGroup: {
                            ...
                            nextHelpDate: dateFormatNYears(mostRecentHelpDate, 4),
        ...
        return {
            ...(isIdGroup
                    ? { idGroup: {
                            eligible: false,
                            mostRecentHelpDate: mostRecentHelpDate,
                            nextHelpDate: dateFormatNYears(mostRecentHelpDate, 2),
                    ...
                    : {
                        addressGroup: {
                            eligible: false,
                            mostRecentHelpDate: dateFormatNYears(mostRecentHelpDate, 0),
                            nextHelpDate: dateFormatNYears(mostRecentHelpDate, 2),
                            message: 'The person is not eligible for help. They have received help within the last two years and must wait until two years from the last help date before receiving help again.',
                            shortMessage: "2 year wait",
                            helpLast2Years: { flag: true }
 ...
 * @returns {boolean}
 */
export function isWithinLastYears(pastDate, currentDate, years) {
...
export function dateFormatNYears(timestamp, years) {
    const dateComponents = timestamp?.split('-').map(Number);
    const targetDate = new Date(Date.UTC(dateComponents[0], dateComponents[1] - 1, dateComponents[2]));

    targetDate.setUTCFullYear(targetDate.getUTCFullYear() + years);
    if (timestamp === '') {
        return '[ NO DATA ]'
    }
    return targetDate.toUTCString().slice(0, 16); // Adjust the slice indices to get your desired date format
}
